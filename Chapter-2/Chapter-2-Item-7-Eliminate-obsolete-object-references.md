...
There’s nothing obviously wrong with this program (but see Item 29 for a generic version). You could test it exhaustively（详尽）, and it would pass every test with flying colors（出色）, but there’s a problem lurking（潜伏）. Loosely speaking, the program has a”memory leak,” which can silently manifest （表现）itself as reduced performance due to increased garbage collector activity or increased memory footprint. In extreme cases, such memory leaks can cause disk paging and even program failure with an OutOfMemoryError, but such failures are relatively rare.
...
**牛皮的，也就是说，如果外部对key的引用没了，这个条目就要被回收了。**
...
**缓存过期清理的策略：background thread和添加时清理，LinkedHashMap 用了后者。**
